// 1 "D:\\Current_Project_15-10-2022\\Current_project_13-12-2022\\Testing_Issues\\Testing_Issues_15-02-2023\\POD_EVRE_GENERIC_IOCL_14-03-2023\\POD_EVRE_GENERIC\\src\\SimpleOcppOperationFactory.cpp"
...
OcppOperation* makeFromTriggerMessage(WebSocketsClient *ws, JsonObject payload) {
  ...
  */
  ...
  if (DEBUG_OUT) {
    Serial.println("[SimpleOcppOperationFactory] makeFromTriggerMessage for message type ");
...
OcppOperation *makeOcppOperation(WebSocketsClient *ws, const char *messageType) {
  ...
  if (!strcmp(messageType, "Authorize")) {
    ...
    if (onRemoteStartTransactionSendConf == NULL) 
      Serial.println("[SimpleOcppOperationFactory] Warning: RemoteStartTransaction is without effect when the sendConf listener is not set. Set a listener which initiates the StartTransaction operation.\n");
    ...
    if (onRemoteStopTransactionSendConf == NULL) 
      Serial.println("[SimpleOcppOperationFactory] Warning: RemoteStopTransaction is without effect when the sendConf listener is not set. Set a listener which initiates the StopTransaction operation.\n");
    ...
    if (onReserveNowSendConf == NULL)
      Serial.println("[SimpleOcppOperationFactory] Warning: ReserveNow is without effect when the sendConf listener is not set. Set a listener which ReserveNow your device.\n");
    ...
    if (onCancelReservationSendConf == NULL)
      Serial.println("[SimpleOcppOperationFactory] Warning: CancelReservation is without effect when the sendConf listener is not set. Set a listener which CancelReservation your device.\n");
    operation->setOnSendConfListener(onCancelReservationSendConf);
  }
  else if (!strcmp(messageType, "Reset")) {
    msg = new Reset();
    if (onResetSendConf == NULL)
      Serial.println("[SimpleOcppOperationFactory] Warning: Reset is without effect when the sendConf listener is not set. Set a listener which resets your device.\n");
  ...
  /**/
    ...
    if (onGetConfigurationSendConf == NULL)
      Serial.println("[SimpleOcppOperationFactory] Warning: GetConfiguration is without effect when the sendConf listener is not set. Set a listener which sends getConfiguration.\n");
  ...
  */
  ...
  /**/
    ...
    if (onClearCacheSendConf == NULL)
      Serial.println("[SimpleOcppOperationFactory] Warning: ClearCache is without effect when the sendConf listener is not set. Set a listener which sends ClearCache.\n");
  ...
  //Added by G. Raja Sumant for Unlock Connector
    ...
    Serial.println("[SimpleOcppOperationFactory] Operation not supported");
...
OcppOperation* makeOcppOperation(WebSocketsClient *ws, OcppMessage *msg){
  if (msg == NULL) {
    Serial.println("[SimpleOcppOperationFactory] in makeOcppOperation(webSocket, ocppMessage): ocppMessage is null!\n");
    return NULL;
  }
  OcppOperation *operation = makeOcppOperation(ws);
  operation->setOcppMessage(msg);
