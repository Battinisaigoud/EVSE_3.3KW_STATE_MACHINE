// 1 "D:\\Current_Project_15-10-2022\\Current_project_13-12-2022\\Testing_Issues\\Testing_Issues_15-02-2023\\POD_EVRE_GENERIC_IOCL_14-03-2023\\POD_EVRE_GENERIC\\src\\CotrolPilot.cpp"
...
/******************************************/
	...
	if(PILOT_reading == V_12){
		...
		EVSE_state = STATE_A;
		Serial.println("[ControlPilot] State A");

	}else if(PILOT_reading == V_9){

		EVSE_state = STATE_B;
		Serial.println("[ControlPilot] State B");

	}else if(PILOT_reading == V_SUS){

		EVSE_state = STATE_SUS;
		Serial.println("[ControlPilot] State SUSPENDED");

	}else if(PILOT_reading == V_DIS){

		EVSE_state = STATE_DIS;
		Serial.println("[ControlPilot] State DISCONNECTED");

	}else if(PILOT_reading == V_6){

		EVSE_state = STATE_C;
		Serial.println("[ControlPilot] State C");

	}else if(PILOT_reading == V_3){

		EVSE_state = STATE_D;
		Serial.println("[ControlPilot] State D");

	}else if(PILOT_reading == V_UNKNOWN){   // can Implement here defaults state

		EVSE_state = STATE_E; //error
		Serial.println("[ControlPilot] State E");

	}else{

		Serial.println("[ControlPilot] Unknown State :ADC value not in range");
	}
