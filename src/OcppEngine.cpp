// 1 "D:\\Current_Project_15-10-2022\\Current_project_13-12-2022\\Testing_Issues\\Testing_Issues_15-02-2023\\POD_EVRE_GENERIC_IOCL_14-03-2023\\POD_EVRE_GENERIC\\src\\OcppEngine.cpp"
...
boolean processWebSocketEvent(uint8_t * payload, size_t length){
  ...
  switch (err.code()) {
    ...
    case DeserializationError::InvalidInput:
        Serial.println("[OcppEngine] Invalid input! Not a JSON\n");
        break;
    case DeserializationError::NoMemory:
        Serial.println("[OcppEngine] Error: Not enough memory\n");
        break;
    default:
        Serial.println("[OcppEngine] Deserialization failed\n");
        break;
  }
  ...
  switch(messageTypeId) {
    ...
    case MESSAGE_TYPE_CALLERROR:
      ...
      break;
    default:
      Serial.println("[OcppEngine] Invalid OCPP message! (though JSON has successfully been deserialized)\n");
      break;
  }
...
void handleReqMessage(JsonDocument *json){
  ...
  if (req == NULL) {
    Serial.println("[OcppEngine] Couldn't make OppOperation from Request. Ignore request.\n");
    return;
  }
...
void handleConfMessage(JsonDocument *json){
  ...
  Serial.println("[OcppEngine] Received CALLRESULT doesn't match any pending operation!\n");
}
...
SmartChargingService* getSmartChargingService(){
  if (ocppEngine_smartChargingService == NULL) {
    Serial.println("[OcppEngine] Error: in OcppEngine, there is no smartChargingService set, but it is accessed!\n");
    //no error catch 
  }
...
ChargePointStatusService *getChargePointStatusService(){
  if (ocppEngine_chargePointStatusService == NULL) {
    Serial.println("[OcppEngine] Error: in OcppEngine, there is no chargePointStatusService set, but it is accessed!\n");
    //no error catch 
  }
...
MeteringService* getMeteringService() {
  if (ocppEngine_meteringService == NULL) {
    Serial.println("[OcppEngine] Error: in OcppEngine, there is no ocppEngine_meteringService set, but it is accessed!\n");
    //no error catch 
  }
