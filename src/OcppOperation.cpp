// 1 "D:\\Current_Project_15-10-2022\\Current_project_13-12-2022\\Testing_Issues\\Testing_Issues_15-02-2023\\POD_EVRE_GENERIC_IOCL_14-03-2023\\POD_EVRE_GENERIC\\src\\OcppOperation.cpp"
...
boolean OcppOperation::sendReq(){
  ...
  if (timeout_active && millis() - timeout_start >= TIMEOUT_CANCEL){
    ...
    Serial.println("[OcppOperation] ");
    Serial.print(ocppMessage->getOcppOperationType());
    Serial.println(" timeout! Cancel operation\n");
    return true;
  }
  ...
  if (DEBUG_OUT) Serial.println("[OcppOperation] Send requirement: ");
...
//#endif
  ...
  if (success) {
    if (DEBUG_OUT) Serial.println("[OcppOperation] (Conf) JSON message: ");
    if (DEBUG_OUT) Serial.print(out);
...
void OcppOperation::print_debug() {
  Serial.println("[OcppOperation] I am a ");
  if (ocppMessage) {
    Serial.print(ocppMessage->getOcppOperationType());
  } else {
    Serial.println("NULL");
