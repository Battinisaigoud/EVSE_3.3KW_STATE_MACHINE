// 1 "D:\\Current_Project_15-10-2022\\Current_project_13-12-2022\\Testing_Issues\\Testing_Issues_15-02-2023\\POD_EVRE_GENERIC_IOCL_14-03-2023\\POD_EVRE_GENERIC\\src\\SmartChargingService.cpp"
...
// SmartChargingService Smart_ChargingService;
...
void SmartChargingService::loop(){
  ...
  if (now() >= nextChange){
    ...
    if (DEBUG_OUT) Serial.println("[SmartChargingService] New Limit! Values: {scheduled at = ");
    if (DEBUG_OUT) printTime(nextChange);
    if (DEBUG_OUT) Serial.println(", nextChange = ");
    if (DEBUG_OUT) printTime(validTo);
    if (DEBUG_OUT) Serial.println(", limit = ");
...
void SmartChargingService::writeOutCompositeSchedule(JsonObject *json){
  Serial.println("[SmartChargingService] Unsupported Operation: SmartChargingService::writeOutCompositeSchedule\n");
}

void SmartChargingService::beginCharging(time_t t, int transactionID){
  if (chargingSessionIsActive) {
    Serial.println("[SmartChargingService] Error: begin new Charging session though there is already running one!\n");
  }
...
void SmartChargingService::endChargingNow(){
  if (!chargingSessionIsActive) {
    Serial.println("[SmartChargingService] Error: end Charging session but there isn't running one!");
  }
...
void SmartChargingService::updateChargingProfile(JsonObject *json){
  ...
  if (DEBUG_OUT) {
    Serial.println("[SmartChargingService] Charging Profile internal model\n");
    chargingProfile->printProfile();
  }

  int stackLevel = chargingProfile->getStackLevel();
  if (stackLevel >= CHARGEPROFILEMAXSTACKLEVEL) {
    Serial.println("[SmartChargingService] Error: Stacklevel of Charging Profile is greater than CHARGEPROFILEMAXSTACKLEVEL\n");
