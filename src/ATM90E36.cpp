// 1 "D:\\Current_Project_15-10-2022\\Current_project_13-12-2022\\Testing_Issues\\Testing_Issues_15-02-2023\\POD_EVRE_GENERIC_IOCL_14-03-2023\\POD_EVRE_GENERIC\\src\\ATM90E36.cpp"
#include "ATM90E36.h"
...
uint32_t gu32under_voltage_skip_count = 0;
...
// added by sai
      ...
      checkForResponse_Disp();
#endif
      // getChargePointStatusService()->setEarthDisconnect(true);
    ...
    {
      // getChargePointStatusService()->setEarthDisconnect(false);
      //  lcd.setCursor(0, 1); // Or setting the cursor in the desired position.
      //  lcd.print("                    ");//Clear the line
      flag_ed_A = false;
    }
  ...
  if (volt < 200)
  {
    ...
    gu32under_voltage_skip_count++;

    //  if (volt<170){ // For LB nagar hubs
    if (gu32under_voltage_skip_count >= 2)
    {
      gu32under_voltage_skip_count = 0;
      // added by sai
      evse_ChargePointStatus = Faulted;
      getChargePointStatusService()->setUnderVoltage(true);
      getChargePointStatusService()->setOverVoltage(false);
    }
...
// err = DWIN_SET(fault_underVolt,sizeof(fault_underVolt)/sizeof(fault_underVolt[0]));
    ...
    // else if((volt>25) && (volt<151))
    // if (volt<120) // For LB nagar hubs
    {
      reasonForStop = Other;
      fault_code_A = EarthDisconnect;
#if LCD_DISPLAY
...
// added by sai
  ...
  else if (volt > 275)
  {
    ...
    // added by sai
    evse_ChargePointStatus = Faulted;
    getChargePointStatusService()->setUnderVoltage(false);
    getChargePointStatusService()->setOverVoltage(true);
...
// added by sai
  ...
  else
  {
    ...
    getChargePointStatusService()->setOverVoltage(false);
    if (DEBUG_OUT)
      Serial.println("Normal Voltage Range.");
    /*
     * @brief To reset the reasonForStop once power is back
     */
    if (reasonForStop == PowerLoss)
    {
      reasonForStop = Local;
    }
    return volt;
...
double ATM90E36::GetLineCurrentA()
{
  ...
  if (curr > 16)
  {
    // added by sai
    evse_ChargePointStatus = Faulted;
    getChargePointStatusService()->setOverCurrent(true);
  ...
  {
    getChargePointStatusService()->setOverCurrent(false);
    return curr;
  }
}
...
double ATM90E36::GetTemperature()
{
  ...
  if (temp < -25)
  {
    getChargePointStatusService()->setUnderTemperature(true);
    getChargePointStatusService()->setOverTemperature(false);
    if (DEBUG_OUT)
  ...
  {
    getChargePointStatusService()->setUnderTemperature(false);
    getChargePointStatusService()->setOverTemperature(true);
    if (DEBUG_OUT)
